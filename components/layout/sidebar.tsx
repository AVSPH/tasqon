...
import {
  LayoutDashboard, Calendar, FolderKanban, Bell,
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto py-4 space-y-0.5 px-2 min-h-0">
        {NAV_ITEMS.map(({ href, label, icon: Icon, badge }) => {
          const active = pathname === href || (href === "/board" && pathname.startsWith("/board"));
          return (
            <Link
              key={href}
              href={href}
            ...
            >
              <Icon className={cn("w-4.5 h-4.5 shrink-0", collapsed ? "w-5 h-5" : "w-[18px] h-[18px]")} />
              {!collapsed && (
        ...
        {!collapsed && (
          <>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-slate-800 truncate">Alex Rivera</p>
