...
import {
  Search, SlidersHorizontal, UserPlus, Bell,
  ChevronDown, Check, X, LogOut, User
} from "lucide-react";
import { cn } from "@/lib/utils";
import { useAppStore } from "@/lib/store";
import { Priority } from "@/lib/types";

const PRIORITY_FILTERS: { label: string; value: Priority | null; color: string }[] = [
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-2.5 mr-2 shrink-0">
        <span className="text-xl">{isAuthenticated ? (activeProject?.emoji ?? "📁") : "🔒"}</span>
        <div>
          <h1 className="font-display font-semibold text-slate-800 text-[15px] leading-tight">
            {isAuthenticated ? (activeProject?.name ?? "Project") : "Sign in required"}
          </h1>
          <p className="text-[11px] text-slate-400 leading-tight">Kanban Board</p>
        </div>
      </div>
        ...
        {showFilter && (
          <div className="absolute top-10 left-0 z-50 bg-white/90 backdrop-blur-xl rounded-xl shadow-modal border border-white/70 p-2 min-w-[160px] animate-scale-in">
            ...
            {PRIORITY_FILTERS.map((f) => (
              <button
                key={f.label}
                onClick={() => { setFilterPriority(f.value); setShowFilter(false); }}
        ...
        <div className="relative">
          ...
          {showInvite && (
            <div className="absolute top-10 right-0 z-50 bg-white/90 backdrop-blur-xl rounded-xl shadow-modal border border-white/70 p-4 w-72 animate-scale-in">
              <p className="font-display font-semibold text-slate-800 text-sm mb-3">Invite to project</p>
              <div className="flex gap-2">
                ...
                />
                <button
                  onClick={() => { setInviteEmail(""); setShowInvite(false); }}
        ...
        >
          ...
          <div className="relative w-full max-w-sm bg-white/90 backdrop-blur-xl rounded-2xl border border-white/70 shadow-modal p-6 animate-scale-in">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display font-semibold text-slate-800">Sign in</h2>
              <button
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={() => {
                  const ok = signIn(authEmail, authPassword);
                  if (!ok) {
                    setAuthError("Invalid credentials. Use test@test.com / test123.");
                    return;
                  }
                  setAuthError("");
                  setAuthEmail("");
